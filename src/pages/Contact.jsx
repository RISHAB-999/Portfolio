...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    // Populate the hidden 'title' variable for EmailJS subject line
    const nameVal = form.current.name?.value || '';
    if (form.current.title) {
    ...
    >
      ...
      <aside className="self-start rounded-2xl border border-white/10 bg-[#11101d]/80 p-5 backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 hover:border-[#5ce1e6] hover:shadow-[0_0_28px_rgba(92,225,230,0.4)] sm:p-6">
        ...
        <div className="flex items-center gap-6">
          <div className="group grid h-32 w-32 flex-none place-items-center overflow-hidden rounded-3xl border-2 border-[#5ce1e6]/60 bg-[#0b0f1f] shadow-[0_0_16px_rgba(92,225,230,0.35)] transition-transform duration-300 ease-out hover:scale-110 cursor-pointer">
            <img src={card.avatar} alt={card.name} decoding="async" className="h-full w-full object-cover transition-transform duration-300 ease-out group-hover:scale-110" />
          </div>
        ...
        <div className="flex flex-col gap-2.5">
          {card.channels.map((ch) => {
            const isResume = ch.type === 'resume';
            const channelClass =
              'group flex items-center gap-3.5 rounded-xl border border-white/10 bg-[#0b0f1f]/50 p-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#5ce1e6] hover:bg-[#5ce1e6] w-full text-left';
            const inner = (
              <>
